...
from modules.bot_utils import SOURCE_WEB_LINK, QUERY_PATTERN_RETRY, get_text_locale
import asyncio
...
async def start_cmd(update: Update, context: CallbackContext) -> None:
    ...
    reply = get_text_locale(user, {"ru": f"Здравствуйте, {user.mention_html()}! Вызовите /help чтобы узнать как пользоваться ботом"},
...
async def gen_audio_cmd(update: Update, context: CallbackContext) -> None:
    ...
    if not context.args:
        reply = get_text_locale(update.effective_user, {"ru": "Ошибка: неверно вызвана команда, предоставьте текст вместе с командой (прим. /gen текст)"},
    ...
    if not validate_text(text):
        reply = get_text_locale(update.effective_user, {"ru": "Ошибка: обнаружен неприемлемый текст"}, "Error: Invalid text detected")
        await update.message.reply_text(reply, reply_to_message_id=reply_id)
        return
...
async def help_cmd(update: Update, context: CallbackContext) -> None:
    ...
    help_msg_ru = ("Использование бота: выбирайте комманды из меню или вводите вручную. Доступный список комманд:\n\n"
                   ...
                   f"Для дополнительной информации обратите внимание на страницу проекта по адресу <a href='{SOURCE_WEB_LINK}'>GitHub</a>")
    reply = get_text_locale(user, {"ru": help_msg_ru}, help_msg)
    await update.message.reply_html(reply)
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update and update.effective_message and update.effective_user:
        reply = get_text_locale(update.effective_user, {"ru": f"{update.effective_user.mention_html()}, к сожалению произошла внутренняя ошибка сервера во время обработки команды, пожалуйста попробуйте еще раз"},
...
async def post_eval_gen_report_error(update: Update, exc) -> None:
    ...
    if update and update.effective_message and update.effective_user:
        reply = get_text_locale(update.effective_user, {"ru": f"{update.effective_user.mention_html()}, к сожалению синтез аудио завершился ошибкой, пожалуйста попробуйте еще раз"},
